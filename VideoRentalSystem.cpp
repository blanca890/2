    ...
    void returnVideo(int customerID, int videoID) {
      ...
      void printAllRentedVideos(const CustomerStore& customerStore, const VideoStore& videoStore) {
        for (const auto& rent : customerRents) {
            int customerID = rent.customerID;
            stack<int> rentedVideos = rent.rentedVideos;
    ...
    do {
        ...
        case 2: {
            ...
            while (true) {
                ...
                cin >> rentAgain;
                if (rentAgain != 'Y' && rentAgain != 'y') {
                    cout << "Rent complete. Returning to main menu." << endl;
                 cout<<"==============================================\n"<<endl;
                    break;
                }
            }
...
case 6: {
    ...
    if (videoStore.checkVideoAvailability(videoID)) {
        ...
        Video availableVideo = videoStore.getVideo(videoID);
        if (availableVideo.videoID != 0) {
            cout << "Movie Title: " << availableVideo.movieTitle << endl;
            cout << "Genre: " << availableVideo.genre << endl;
            cout << "Production: " << availableVideo.production << endl;
            cout << "Number of Copies: " << availableVideo.numCopies << endl;
        } else {
            cout << "Error: Video information not found." << endl;
        }
    } else {
